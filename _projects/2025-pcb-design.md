...
Typing them in Google Docs requires navigating through *Insert → Special Characters → Search*, which quickly becomes tedious.

I wondered: what if there were a **small dedicated keyboard for Greek symbols**?

At the same time, I wanted to learn PCB design. As a mechanical engineering student, most of my electronics experience had been with Arduinos and breadboards, which are functional but messy. After completing the Blinky 5.0 tutorial to fabricate my first PCB, I wanted to push further and design a **clean, purpose-built embedded device**.

This project is a **prototype capacitive touch keyboard** for Greek symbols.  
The current version includes four capacitive pads with LED feedback, powered by a Raspberry Pi Pico 2 W.
...
To make the sensing stable and predictable, each pad discharges through a **pull-down resistor**, which controls the discharge rate. Without this resistor the pads would discharge unpredictably, making it difficult to tune the sensing thresholds.
...
- Avoiding ground pours directly beneath the capacitive pads
- Keeping PCB mounting screws at least ~5 mm away from pads
- Separating high-current 5 V traces from the sensing lines
- Avoiding long parallel runs between sensing traces and power lines
- Keeping traces clear of the Pico W antenna region for future Bluetooth functionality
...
6. Hand-assemble and solder the board
...
The LEDs and firmware infrastructure are working, and the board successfully runs LED animations and system tests.
...
The next revision of the keyboard will include:

- Integrated **LRA haptic feedback** for tactile confirmation of button presses
- Expanded number of capacitive pads
- Direct integration of the **RP2350 microcontroller** instead of using the Pico module
- Wireless functionality using the Pico W radio
